...
class CRNNNet(object):
    ...
    def net(self, inputs, is_test=False, width=None):
        ...
        def BLSTM(inputs, num_hidden, num_layers, seq_len, num_classes, reuse=None):
            ...
            stack = tf.contrib.rnn.core_rnn_cell.MultiRNNCell(
                [(tf.contrib.rnn.core_rnn_cell.LSTM(num_hidden, state_is_tuple=True)) for i in range(num_layers)],
                state_is_tuple=True)
        ...
        def conv2d(inputs, i, batchNormalization=False):
            ...
            if i != 6:
                net = slim.conv2d(inputs, nOut, kernel_size, stride=stride, padding="SAME", scope='conv_{0}'.format(i),
                                  activation_fn=None)
            else:
